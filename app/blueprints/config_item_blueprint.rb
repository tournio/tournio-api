class ConfigItemBlueprint < Blueprinter::Base
  ...
  field :value do |c, _|
    if %w(email_in_dev display_capacity skip_stripe publicly_listed accept_payments stripe_receipts enable_unpaid_signups enable_free_entries).include?(c.key)
